...
if [ $? != 0 ]; then
    ...
    cppcheck -q  -f -I $directories  --enable=style --enable=performance --enable=portability --enable=information --enable=missingInclude --enable=warning -i $PACKETGRAPH_ROOT/switch/src/switch.c $c_filelist 
    if [ $? != 0 ]; then
