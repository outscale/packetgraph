...
struct pg_graph *pg_graph_new(const char *name, struct pg_brick *explore)
{
	...
	ret->all = g_hash_table_new(g_str_hash, g_str_equal);
	ret->pollable = NULL;

	if (explore) {
		pg_graph_explore_ptr(ret, explore, &error);
		if (error)
			pg_error_free(error);
	}
...
static inline GList *explore_add(GList *todo,
				 GList *done,
				 struct pg_brick *brick)
{
	if (brick && !g_list_find(todo, brick) && !g_list_find(done, brick))
		return g_list_prepend(todo, brick);
	return todo;
}
					    ...
					    struct pg_brick *b)
{
	if (b->type == PG_MONOPOLE)
		return explore_add(todo, done, b->side.edge.link);
	for (int i = 0; i < MAX_SIDE; i++) {
		if (b->type == PG_DIPOLE) {
			todo = explore_add(todo, done, b->sides[i].edge.link);
		} else {
			for (int j = 0; j < b->sides[i].max; j++)
				todo = explore_add(todo, done,
						   b->sides[i].edges[j].link);
		}
	}
	return todo;
}
			 ...
			 struct pg_error **error)
{
	if (!brick) {
		...
		if (!brick) {
			*error = pg_error_new("Graph %s is empty", graph->name);
			return -1;
		}
	}
	...
	todo = g_list_prepend(todo, brick);
	while (todo != NULL) {
...
int pg_graph_sanity(struct pg_graph *graph, struct pg_error **error)
{
	...
	struct pg_graph *g = pg_graph_new("-", brick);
	int ret = -1;
...
static inline void graph_pop(struct pg_graph *graph, struct pg_brick *b)
{
	g_hash_table_remove(graph->all, b->name);
	if (b->poll)
				...
				struct pg_error **error)
{
	...
	if (pg_graph_explore_ptr(graph, qw, error) < 0)
		return NULL;
	return pg_graph_new(east_graph_name, qe);
}
...
static inline GList *get_all_queues(struct pg_graph *g)
{
	...
	while (h) {
		if (g_str_equal(((struct pg_brick *)h->data)->ops->name,
				"queue"))
