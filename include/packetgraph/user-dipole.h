...
/**
 * @output	side on which the packets will be send
 * @pkts_mask	represent inputs and output packets,
 *		if you want to block a packet, just remove it's index from
 *		@pkts_mask
 *		Example:
 *		a pkts_mast equal to 0x6 mean that @pkts[0] contain
 *		no valide packet.
 *		but @pkts[1] and @pkts[2] can be safely acess and modifie
 *		if then you set @pkts_mast to 0x4, it mean that pkts[1] must
 *		be free, and must be block by this brick.
