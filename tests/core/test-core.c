						  ...
						  struct pg_brick *east[4])
{
	...
	for (i = 0; i < 4; i++) {
		ret = pg_brick_link(west[i], middle, &error);
		g_assert(ret == 0);
		g_assert(!error);
		ret = pg_brick_link(middle, east[i], &error);
		g_assert(ret == 0);
		g_assert(!error);
	}

	refcount = pg_brick_refcount(middle);
	g_assert(refcount == 9);
	for (i = 0; i < 4; i++) {
		refcount = pg_brick_refcount(west[i]);
		g_assert(refcount == 2);
		refcount = pg_brick_refcount(east[i]);
		g_assert(refcount == 2);
	}
	...
	g_assert(!error);
	refcount = pg_brick_refcount(west[0]);
	g_assert(refcount == 1);
	for (i = 1; i < 4; i++) {
		refcount = pg_brick_refcount(west[i]);
		g_assert(refcount == 2);
	...
	refcount = pg_brick_refcount(middle);
	g_assert(refcount == 6);
	refcount = pg_brick_refcount(east[0]);
	g_assert(refcount == 1);
	for (i = 1; i < 4; i++) {
		refcount = pg_brick_refcount(east[i]);
		g_assert(refcount == 2);
	...
	refcount = pg_brick_refcount(west[3]);
	g_assert(refcount == 2);
	refcount = pg_brick_refcount(middle);
	g_assert(refcount == 4);
	for (i = 0; i < 2; i++) {
	...
	refcount = pg_brick_refcount(east[3]);
	g_assert(refcount == 2);
