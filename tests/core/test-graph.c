...
static void test_graph_lifecycle(void)
{
	...
	g = pg_graph_new("g1", NULL);
	g_assert(g);
	g_assert(g_str_equal(pg_graph_name(g), "g1"));
	g_assert(pg_graph_count(g) == 0);
	pg_graph_destroy(g);

	g = pg_graph_new("g2", NULL);
	g_assert(g);

	for (int i = 0; i < 10; i++) {
		g_assert(g_str_equal(pg_graph_name(g), "g2"));
		g_assert(pg_graph_count(g) == 0);
...
static void test_graph_explore(void)
{
	...
	g1 = pg_graph_new("graph1", nop[0]);
	g_assert(g1);
	g_assert(pg_graph_count(g1) == 10);
	g2 = pg_graph_new("graph2", queue[2]);
	g_assert(g2);
...
static void test_graph_sanity(void)
{
	 ...
	 * [nop0]--[nop1]--[nop2]--[nop3]
	 *
	 * and break it, should not be sane anymore
	 */
	struct pg_graph *g;
	struct pg_error *error = NULL;
...
#	define NB_PKTS 64
	...
	g_assert(!error);
	g = pg_graph_new("graph", hub);
	g_assert(g);
	g_assert(pg_graph_count(g) == 4);
	trash = pg_graph_new("trash", NULL);
	g_assert(!error);
...
static void test_graph_complex_merge(void)
{
	...
	g_west = pg_graph_new("graph-west", sw[0]);
	g_assert(g_west);
	g_assert(pg_graph_count(g_west) == 7);
	g_east = pg_graph_new("graph-east", sw[1]);
	g_assert(g_west);
	g_assert(pg_graph_count(g_west) == 7);
