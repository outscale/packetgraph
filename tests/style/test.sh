...
c_filelist_tests=$(find $PACKETGRAPH_ROOT/tests/ -path $PACKETGRAPH_ROOT/src/npf -prune -o -type f -name "*.c" -printf %p\ )
h_filelist_tests=$(find $PACKETGRAPH_ROOT/tests/ -path $PACKETGRAPH_ROOT/src/npf -prune -o -type f -name "*.h" -printf %p\ )
c_filelist_examples=$(find $PACKETGRAPH_ROOT/examples/ -path $PACKETGRAPH_ROOT/src/npf -prune -o -type f -name "*.c" -printf %p\ )
# checkpatch tests
...
$PACKETGRAPH_ROOT/tests/style/checkpatch.pl --ignore TRAILING_SEMICOLON,SPLIT_STRING --show-types --no-tree -q -f $c_filelist_examples
if [ $? != 0 ]; then
