...
static void  funct_un_subscribe( struct vtep_state *state, struct vtep_port *port,
				...
				struct pg_error **errp)
{
  ...
  number = 1;
  funct_un_subscribe(state, port, multicast_ip, errp, number);
}
...
static void  funct_un_subscribe( struct vtep_state *state,
				 struct vtep_port *port,
				 uint32_t multicast_ip,
				 struct pg_error **errp,
				 int  number)
